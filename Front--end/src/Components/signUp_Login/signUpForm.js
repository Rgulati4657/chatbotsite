...
import axios from "axios";   // pending
const SignUpForm = () => {
  ...
  const handleStep2Submit = async (e) => {
    ...
    // Now submit both step 1 and step 2 data to the database
    dispatch(authActions.signupValidate({ formType: "company" }));
    // You can now trigger any backend call here to store all the form data
  ...
  return (
    <div className={styles.form_main_div}>
      ...
      <div className={styles.container}>
        ...
        <form className={styles.form} > {/* action="/api/auth/signup" */} 
          {/* Step 1: User Info */}
          {!step1Complete && (
            <>
